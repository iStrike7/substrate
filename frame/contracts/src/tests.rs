...
#[test]
fn lazy_removal_capped_by_weight() {
	let (code, hash) = compile_module::<Test>("self_destruct").unwrap();
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		let subsistence = ConfigCache::<Test>::subsistence_threshold_uncached();
		let _ = Balances::deposit_creating(&ALICE, 10 * subsistence);
		...
		let (_weight_per_key, max_keys) = Storage::<Test>::deletion_budget(1, weight_limit);

		println!("max_keys: {}", max_keys);

		let vals: Vec<_> = (0..max_keys+100).map(|i| {
			(blake2_256(&i.encode()), (i as u32).encode())
		})
		.collect();
		...
		println!("weight_used: {:?}", weight_used);

		// But value should be still there as the lazy removal did not run, yet.
		for val in vals {
			assert_matches!(child::get::<u32>(trie, &blake2_256(&val.0)), None);
		}
	});
}
